    ...
    if opcion == 1:
        ...
        if nombre != "":
          if existe_pais(nombre): #Verificamos que el pais no exista ya en la base de datos
            eleccion = str(input("Ese país ya existe en la base de datos, le gustaría acceder a la herramienta de actualizador de datos?[S/N]: ")).lower()
            if eleccion == "s":
                actualizar_datos()
            elif eleccion == "n":
            ...
            cantidad_habitantes = validador(input("ingrese la cantidad de habitantes (Expresado en numeros y sin ',' o '.'): "))
            superficie = validador(input("Ingrese la superficie del país expresada en metros cuadrados (expresado unicamente en numeros y sin caracteres especiales): "))
